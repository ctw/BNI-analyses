...
evs = np.load(
    '/home/ctw/Christoph/Analyses/BNI/ir/data/bni_evs_ir.npz')['evs'].view(
        np.recarray)
...
bad_expstr = ['s51e4ir', 's46e8ir']
#  Please note that apparently we do not have actual raw recordings
#  for s51e4ir. We also do not have spike sorting output for
#  s46e8ir. I can try and generate that if you think it will be
#  important.
...
# glob(rhino_root+'/scratch/josh/BniData/Subjects/*/analysis/*/kk')
...
for area in all_areas:
    ...
    for i, av in enumerate(all_vecs):
        ...
        indices = {'allevs': np.arange(len(av['events'])),
                   ...
                   'category2_img': {}, 'category2_txt': {}}
        for stim_desc in np.unique(av['events']['stim_file']):
            indices['description'][stim_desc] = indices['allevs'][
                av['events']['stim_file'] == stim_desc]
        ...
        for stim_cat2 in np.unique(av['events']['stim_category2']):
            if stim_cat2 == 'INVALID':
                continue
            indices['category2'][stim_cat2] = indices['allevs'][
                av['events']['stim_category2'] == stim_cat2]
            indices['category2_img'][stim_cat2] = indices['allevs'][
                (av['events']['stim_category2'] == stim_cat2) &
                (av['events']['stim_imgtxt'] == 'image')]
            indices['category2_txt'][stim_cat2] = indices['allevs'][
                (av['events']['stim_category2'] == stim_cat2) &
                (av['events']['stim_imgtxt'] == 'text')]
            # assert len(indices['category'][stim_cat])%2 == 0, 'cat even'
        sim_mean = {}
        sim_perm_mean = {}
        for comptype in ['description', 'category', 'category2',
                         'category2_img', 'category2_txt']:
        ...
        # compare images to text presentations:
        sim_mean['category2_imgtxt'] = np.ones(permutations)*np.nan
        sim_perm_mean['category2_imgtxt'] = np.ones(permutations)*np.nan
        for p in range(permutations):
            # print(p+1, '/', permutations)
            indx1 = []
            indx2 = []
            for stim_desc in indices['category2']:
                indxlen = np.min([len(indices['category2_img'][stim_desc]),
                                  len(indices['category2_txt'][stim_desc])])
                if indxlen < 1:
                    continue
                indx1.extend(rng.permutation(indices['category2_img'][
                    stim_desc])[:indxlen])
                indx2.extend(rng.permutation(indices['category2_txt'][
                    stim_desc])[:indxlen])
            assert len(indx1) == len(indx2), 'len indx1==indx2 imgtxt'
            # indx1 = np.array(ind
            sim_mean['category2_imgtxt'][p] = np.tanh(np.nanmean(np.arctanh(
                np.diag(np.dot(norm_vecs[:, indx1].T, norm_vecs[:, indx2])))))
            indx2_perm = rng.permutation(indx2)
            sim_perm_mean['category2_imgtxt'][p] = np.tanh(np.nanmean(
                np.arctanh(np.diag(np.dot(norm_vecs[:, indx1].T,
                                          norm_vecs[:, indx2_perm])))))
...
# indx2a = []
# for stim_desc in indices[comptype]:
#     indxlena = np.int32(len(indices[comptype][stim_desc])/2)
#     indxa = rng.permutation(indices[comptype][stim_desc])
#     indx1a.extend(indxa[:indxlena])
#     indx2a.extend(indxa[-indxlena:])
#     print(stim_desc, indxlena, len(indxa))
...
# area = 'all'
# for area in all_areas:
bad_expstr = ['s34e4ir', 's41e3ir', 's41e4ir',
              's44e8ir', 's46e6ir', 's50e6ir']
...
pickle.dump(all_ps, open('./data/all_ps_zmean_cat2.pickle', 'xb'), -1)
...
for area in all_ps:
    comparisons[area] = {'desc': {}, 'cat': {}, 'cat2': {}, 'cat2img': {},
    ...
    for sm, spm in zip(all_ps[area]['sim_means'],
        ...
        comparisons[area]['cat']['d_prime'].append(
            (sm['category'].mean()-spm['category'].mean())/spm['category'].std())
        #
        comparisons[area]['cat2']['ks'].append(ks_2samp(sm['category2'], spm['category2']))
        comparisons[area]['cat2']['mean'].append(sm['category2'].mean())
        comparisons[area]['cat2']['mean_perm'].append(spm['category2'].mean())
        comparisons[area]['cat2']['std_perm'].append(spm['category2'].std())
        comparisons[area]['cat2']['diff'].append(
            sm['category2'].mean()-spm['category2'].mean())
        comparisons[area]['cat2']['d_prime'].append(
            (sm['category2'].mean()-spm['category2'].mean())/spm['category2'].std())
        #
        comparisons[area]['cat2img']['ks'].append(ks_2samp(
            sm['category2_img'], spm['category2_img']))
        comparisons[area]['cat2img']['mean'].append(
            sm['category2_img'].mean())
        comparisons[area]['cat2img']['mean_perm'].append(
            spm['category2_img'].mean())
        comparisons[area]['cat2img']['std_perm'].append(
            spm['category2_img'].std())
        comparisons[area]['cat2img']['diff'].append(
            sm['category2_img'].mean()-spm['category2_img'].mean())
        comparisons[area]['cat2img']['d_prime'].append(
            (sm['category2_img'].mean()-spm['category2_img'].mean()) /
            spm['category2_img'].std())
        #
        comparisons[area]['cat2txt']['ks'].append(ks_2samp(
            sm['category2_txt'], spm['category2_txt']))
        comparisons[area]['cat2txt']['mean'].append(
            sm['category2_txt'].mean())
        comparisons[area]['cat2txt']['mean_perm'].append(
            spm['category2_txt'].mean())
        comparisons[area]['cat2txt']['std_perm'].append(
            spm['category2_txt'].std())
        comparisons[area]['cat2txt']['diff'].append(
            sm['category2_txt'].mean()-spm['category2_txt'].mean())
        comparisons[area]['cat2txt']['d_prime'].append(
            (sm['category2_txt'].mean()-spm['category2_txt'].mean()) /
            spm['category2_txt'].std())
        #
        comparisons[area]['cat2imgtxt']['ks'].append(ks_2samp(
            sm['category2_imgtxt'], spm['category2_imgtxt']))
        comparisons[area]['cat2imgtxt']['mean'].append(
            sm['category2_imgtxt'].mean())
        comparisons[area]['cat2imgtxt']['mean_perm'].append(
            spm['category2_imgtxt'].mean())
        comparisons[area]['cat2imgtxt']['std_perm'].append(
            spm['category2_imgtxt'].std())
        comparisons[area]['cat2imgtxt']['diff'].append(
            sm['category2_imgtxt'].mean()-spm['category2_imgtxt'].mean())
        comparisons[area]['cat2imgtxt']['d_prime'].append(
            (sm['category2_imgtxt'].mean()-spm['category2_imgtxt'].mean()) /
            spm['category2_imgtxt'].std())
        #
        comparisons[area]['desc-cat']['ks'].append(
...
for area in comparisons:
    print(area, '\t',
          np.round(np.nanmean(comparisons[area]['desc']['d_prime']), 2),
          np.round(np.nanmean(comparisons[area]['cat']['d_prime']), 2),
          np.round(np.nanmean(comparisons[area]['cat2']['d_prime']), 2),
          np.round(np.nanmean(comparisons[area]['cat2img']['d_prime']), 2),
          np.round(np.nanmean(comparisons[area]['cat2txt']['d_prime']), 2),
          np.round(np.nanmean(comparisons[area]['cat2imgtxt']['d_prime']), 2))
...
fig, axs = plt.subplots(1, 1)
for i, area in enumerate(areas):
    axs.plot(i*5.5-0.35+(np.random.uniform(size=len(comparisons[area]['desc']['d_prime']))-0.5)*0.3,
             comparisons[area]['desc']['d_prime'], 'ko', alpha=0.5)
    axs.boxplot(np.array(comparisons[area]['desc']['d_prime'])[
        np.isfinite(comparisons[area]['desc']['d_prime'])], positions=[i*5.5-0.35],
                notch=True, bootstrap=10000, widths=0.65, showfliers=False,
                whiskerprops=dict(lw=0), capprops=dict(lw=0))
    axs.plot(i*5.5+0.35+(np.random.uniform(size=len(comparisons[area]['cat']['d_prime']))-0.5)*0.3,
             comparisons[area]['cat']['d_prime'], 'bo', alpha=0.5)
    axs.boxplot(np.array(comparisons[area]['cat']['d_prime'])[
        np.isfinite(comparisons[area]['cat']['d_prime'])], positions=[i*5.5+0.35],
                notch=True, bootstrap=10000, widths=0.65, showfliers=False,
                whiskerprops=dict(lw=0), capprops=dict(lw=0))
    #
    axs.plot(i*5.5+0.35*3+(np.random.uniform(size=len(comparisons[area]['cat2']['d_prime']))-0.5)*0.3,
             comparisons[area]['cat2']['d_prime'], 'bo', alpha=0.5)
    axs.boxplot(np.array(comparisons[area]['cat2']['d_prime'])[
        np.isfinite(comparisons[area]['cat2']['d_prime'])], positions=[i*5.5+0.35*3],
                notch=True, bootstrap=10000, widths=0.65, showfliers=False,
                whiskerprops=dict(lw=0), capprops=dict(lw=0))
    
    axs.plot(i*5.5+0.35*5+(np.random.uniform(size=len(comparisons[area]['cat2img']['d_prime']))-0.5)*0.3,
             comparisons[area]['cat2img']['d_prime'], 'bo', alpha=0.5)
    axs.boxplot(np.array(comparisons[area]['cat2img']['d_prime'])[
        np.isfinite(comparisons[area]['cat2img']['d_prime'])], positions=[i*5.5+0.35*5],
                notch=True, bootstrap=10000, widths=0.65, showfliers=False,
                whiskerprops=dict(lw=0), capprops=dict(lw=0))
    #
    axs.plot(i*5.5+0.35*7+(np.random.uniform(size=len(comparisons[area]['cat2txt']['d_prime']))-0.5)*0.3,
             comparisons[area]['cat2txt']['d_prime'], 'bo', alpha=0.5)
    axs.boxplot(np.array(comparisons[area]['cat2txt']['d_prime'])[
        np.isfinite(comparisons[area]['cat2txt']['d_prime'])], positions=[i*5.5+0.35*7],
                notch=True, bootstrap=10000, widths=0.65, showfliers=False,
                whiskerprops=dict(lw=0), capprops=dict(lw=0))
    #
    axs.plot(i*5.5+0.35*9+(np.random.uniform(size=len(comparisons[area]['cat2imgtxt']['d_prime']))-0.5)*0.3,
             comparisons[area]['cat2imgtxt']['d_prime'], 'bo', alpha=0.5)
    axs.boxplot(np.array(comparisons[area]['cat2imgtxt']['d_prime'])[
        np.isfinite(comparisons[area]['cat2imgtxt']['d_prime'])], positions=[i*5.5+0.35*9],
                notch=True, bootstrap=10000, widths=0.65, showfliers=False,
                whiskerprops=dict(lw=0), capprops=dict(lw=0))
