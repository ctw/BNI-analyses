...
stims375 = {'animals': [],
            ...
            'Buildings': [],
            'neutral_faces': ['FACE3'], 'scenes-indoor': [],
            'Tools': []}
for sf in stims375stimfiles:
    fn = sf.split('/')[-1].split('.')[0]
    if fn[0].upper() == 'A':
        stims375['animals'].append(fn)
    elif '_' in fn:
        stims375['famous'].append(fn)
    elif fn[0].upper() == 'B':
        stims375['Buildings'].append(fn)
    elif fn[0] == 'i':
        stims375['scenes-indoor'].append(fn)
    elif fn[0] == 't':
        stims375['Tools'].append(fn)
...
oi_catmap = pd.read_csv(
    '/home/ctw/fusemounts/rhino/scratch/josh/BniData/Analysis/'+
    'ObjectInvariance/catmap.txt', sep='\t')

ir_catmap = pd.read_csv(
    '/home/ctw/fusemounts/rhino/scratch/josh/BniData/Analysis/'+
    'IR/catmap.txt', sep='\t')


dat_files = {}
dat_files['ir'] = sorted(glob(data_dir+'/s???/data/s*e*ir/IRLog_*'))

#exp_dict = {}
ev_raw_dtypes = [('time', int), ('code', int), ('param', int)]

evs = None
# s42e4ir, s49e3ir: synch timing is off
bad_exps = ['s42e4ir', 's49e3ir'] 
for datfile in dat_files['ir']:
    ...
    if 'fixed' in datfile:
        raise ValueError('No fixed files in OI')
            ...
            if game_dict_parse:
                ...
                if 'Image' in line:
                    ...
                    if filename in np.array(pictureprops['StandardName'],
                        ...
                        tmp_dict['stim_oname'] = pp[
                            'OriginalName'].to_list()[0]
                    elif filename.upper() in [fn.split('.')[0].upper()
                                              for fn in np.array(oi_catmap[
                                                      'name'], 'U32')]:
                        indx = filename.upper() == np.array(
                            [fn.split('.')[0].upper()
                             for fn in np.array(oi_catmap['name'], 'U32')])
                        cm = oi_catmap[indx]
                        assert len(cm) == 1, 'len(oi_cm) == 1'
                        tmp_dict['stim_cat'] = cm['category'].to_list()[0]
                    elif filename.upper() in [fn.split('.')[0].upper()
                                              for fn in np.array(ir_catmap[
                                                      'name'], 'U32')]:
                        indx = filename.upper() == np.array(
                            [fn.split('.')[0].upper()
                             for fn in np.array(ir_catmap['name'], 'U32')])
                        cm = ir_catmap[indx]
                        assert len(cm) == 1, 'len(ir_cm) == 1'
                        tmp_dict['stim_cat'] = cm['category'].to_list()[0]
                    else:
                ...
                #     else:
                #         exp_dict[code] = tmp_dict.copy()
                elif 'Object Invariance' in line:
                    exp_params = line.split()
                    code = np.int(exp_params[0])
                    tmp_dict = {}
                    if 'Started' in line:
                        tmp_dict['evtype'] = 'ir_start'
                    elif 'Stopped' in line:
                        tmp_dict['evtype'] = 'ir_stop'
                    else:
                        raise ValueError('Unknown exp parameter: '+line)
                    if code in exp_dict:
                        assert exp_dict[code] == tmp_dict, \
    ...
    if exp_dict[sessevs_raw_rec[indx]['code']]['evtype'] != 'ir_start':
...
np.savez_compressed('./data/bni_evs_ir.npz', evs=evs)
...
cats2 = {}
for sf in np.unique(evs['stim_file']):
    # if sf == 'MountRushmore_txt.jpg':
    #     continue
    if '_txt' in sf:
        if sf.split('_txt')[0] == 'GeorgeClooney':
            key = 'CLO'
        elif sf.split('_txt')[0] == 'JenniferAniston':
            key = 'ANI'
        elif sf.split('_txt')[0] == 'JenniferLopez':
            key = 'LOP'
        else:
            key = sf[:3].upper()
        if key in cats2:
            ...
            cats2[key] = sf.split('_txt')[0]


cat2_list = []
imgtxt_list = []
for sf in evs['stim_file']:
    if 'GeorgeClooney' in sf:
        c2 = cats2['CLO']
    elif 'JenniferAniston' in sf:
        c2 = cats2['ANI']
    elif 'JenniferLopez' in sf:
        c2 = cats2['LOP']
    #if sf == 'MountRushmore_txt.jpg':
    #     c2 = cats2['RUS']
    else:
        key = sf[:3].upper()
        if key not in cats2:
            # GeorgeWBush only has 2 images
            print(key, 'not a category: ', sf)
            imgtxt_list.append('INVALID')
            cat2_list.append('INVALID')
            continue
        c2 = cats2[key]
    if '_txt' in sf:
        imgtxt_list.append('text')
    else:
        imgtxt_list.append('image')
    cat2_list.append(c2)

evs = np.lib.recfunctions.append_fields(evs, ['stim_category2', 'stim_imgtxt'], [cat2_list, imgtxt_list], usemask=False, asrecarray=True)
# evs = np.lib.recfunctions.append_fields(evs, 'stim_imgtxt', imgtxt_list, usemask=False, asrecarray=True)
np.savez_compressed('./data/bni_evs_ir.npz', evs=evs)
