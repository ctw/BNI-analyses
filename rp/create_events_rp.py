...
pictureprops = pd.read_csv(
    '/home/ctw/fusemounts/rhino/scratch/josh/BniData/Analysis/RP_Analysis/'+
    'PictureProperties.txt', sep='\t')

dat_files = {}
dat_files['rp'] = sorted(glob(data_dir+'/s???/data/s*e*rp/RPLog_*'))

for df in dat_files['rp']:
    if df[:-4]+'_fixed.txt' in dat_files['rp']:
        dat_files['rp'].remove(df)
...
for datfile in dat_files['rp']:
            ...
            if game_dict_parse:
                ...
                if 'Image' in line:
                    ...
                    if code in exp_dict:
                        ...
                        exp_dict[code] = tmp_dict.copy()
                elif 'Rossion' in line:
                    exp_params = line.split()
                    code = np.int(exp_params[0])
                    tmp_dict = {}
                    if 'Started' in line:
                        tmp_dict['evtype'] = 'rp_start'
                    elif 'Stopped' in line:
                        tmp_dict['evtype'] = 'rp_end'
                    else:
                        raise ValueError('Unknown exp parameter: '+line)
                    if code in exp_dict:
                        assert exp_dict[code] == tmp_dict, \
                    ...
                    raise ValueError(
                        'There should be no fixation in RP: '+line)
                    # exp_params = line.split()
                    # code = np.int(exp_params[0])
    ...
    times = []
    time_thresh = 5
    # check that code 1 corresponds to 'synch_end' event so below
    ...
    for i,row in enumerate(sessevs_raw_rec):
        if row['code'] != 1:  # synch_end code as tested by above assertion
            continue
        if exp_dict[sessevs_raw_rec[i-1]['code']]['evtype'] == 'rp_end':
            continue
    ...
    if exp_dict[sessevs_raw_rec[indx]['code']]['evtype'] != 'rp_start':
    ...
    while indx < len(sessevs_raw_rec):
        ...
        sessevs.append(
            (exp, subj, expnum, sess, expstr, ev_dict['stim_time'],
             ...
             ev_dict['stim_file'], ev_dict['stim_description'],
             ev_dict['stim_category'], ev_dict['resp_early'],
...
np.savez_compressed('./data/bni_evs_rp.npz', evs=evs)
