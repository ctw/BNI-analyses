...
# import sys
# sys.path.append('/home/ctw/data/BNI/data/ReberEtAl2019/mat7.3-master')
# import mat73
# from scipy.io import loadmat
# #import h5py
# import hdf5storage

# datafn = '/home/ctw/data/BNI/data/ReberEtAl2019/zvals_trials.mat'
# clusterlookupfn = '/home/ctw/data/BNI/data/ReberEtAl2019/cluster_lookup.mat'
# mat = mat73.loadmat(datafn)
# # cl = mat73.loadmat(clusterlookupfn)
# # data = h5py.File(datafn, 'r')

# cl2 = loadmat(clusterlookupfn)['cl2']
# columns = np.array([c[0] for c in cl2[0]])
# tab = []
# for row in cl2[1:]:
#     tab.append([c.squeeze().tolist() if len(c) > 0 else 'nonMTL' for c in row])
#     # tab.append([np.squeeze(c.tolist()) if len(c) > 0 else 'nonMTL' for c in row])

datfn = '/home/ctw/data/BNI/data/ReberEtAl2019/zvals_trials.h5'
data = h5py.File(datfn, 'r')
...
for subj in np.unique(data['cluster_lookup'][:]['subjid']):
    for sess in np.unique(data['cluster_lookup'][:][data['cluster_lookup'][:][
        ...
        filt = (data['cluster_lookup'][:]['subjid']==subj) & (data['cluster_lookup'][:]['sessid']==sess)
        events = {}
        channels = {}
        clusters = {}
        clusterIDs = {}
        ...
        firing_rate = {}
        # zval_filt = {}
        all_areas = np.unique(data['cluster_lookup'][:]['regionname'])
        ...
        for cluster in data['cluster_lookup'][filt]:
            ...
            area = cluster['regionname']
            # zval_filt[area].append(filt & (data['cluster_lookup'][:]['channo']==channum) & (
            #     data['cluster_lookup'][:]['cluster_num']==cluster_num))
            channels[area].append(channum)
            clusters[area].append(cluster_num)
            clusterIDs[area].append(
                'reberCLUSID_'+str(channum)+'cl'+str(cluster_num))
            ####spikesums[ci['area'][0]].append(hist[::2])
        for area in all_areas:  # channels:
            area_filt = filt & (data['cluster_lookup'][:]['regionname']==area)
            if area_filt.sum() > 0:
                spike_zvals[area] = data['zvals'][area_filt, :]
                firing_rate[area] = data['fr'][area_filt, :]
                channels[area] = np.array(channels[area])
                clusters[area] = np.array(clusters[area])
                clusterIDs[area] = np.array(clusterIDs[area])
            else:
                spike_zvals[area] = np.array([])
                firing_rate[area] = np.array([])
                channels[area] = np.array([])
                clusters[area] = np.array([])
                clusterIDs[area] = np.array([])
        all_vecs.append({'expstr': expstr, 'clusters': clusters,
                         'clusterIDs': clusterIDs, 'channels': channels,
                         'spike_zvals': spike_zvals,
...
for area in all_areas:
    ...
    for i, av in enumerate(all_vecs):
        ...
        indices = {'allevs': np.arange(len(data['cat_lookup'])),
                   'description': {}, 'category': {}}
        for stim_desc in np.unique(data['stim_lookup']):
            indices['description'][stim_desc] = indices['allevs'][
                np.ravel(data['stim_lookup'][:] == stim_desc)]
            # assert len(indices['description'][stim_desc])%2 == 0, 'desc even'
        for stim_cat in np.unique(data['cat_lookup']):
            indices['category'][stim_cat] = indices['allevs'][
                np.ravel(data['cat_lookup'][:] == stim_cat)]
        ...
        for comptype in ['description', 'category']:
            ...
            for p in range(permutations):
                ...
                # indx1 = np.array(ind
                # sim_mean[comptype][p] = np.nanmean(np.diag(np.dot(
                #     norm_vecs[:, indx1].T, norm_vecs[:, indx2])))
                ...
                # sim_perm_mean[comptype][p] = np.nanmean(np.diag(np.dot(
                #     norm_vecs[:, indx1].T, norm_vecs[:, indx2_perm])))
...
scipy.stats.ks_2samp(all_ps['all']['sim_means'][0]['description'], all_ps['all']['sim_perm_means'][0]['description'])
scipy.stats.ks_2samp(all_ps['all']['sim_means'][0]['category'], all_ps['all']['sim_perm_means'][0]['category'])
...
#'expstr': all_ps[area]['expstr'], 
for area in comparisons:
    print(area, np.nanmean(comparisons[area]['desc']['d_prime']),
          np.nanmean(comparisons[area]['cat']['d_prime']))

expstr0 = all_ps['all']['expstr'][0]
expstr28 = all_ps['all']['expstr'][28]
for area in comparisons:
    if expstr0 not in all_ps[area]['expstr']:
        continue
    indx = np.where(np.array(all_ps[area]['expstr']) == expstr0)[0][0]
    print(area, indx, 'desc', np.round(comparisons[area]['desc']['d_prime'][
        indx], 2), np.round(comparisons[area]['desc']['mean'][indx], 2),
          np.round(comparisons[area]['desc']['mean_perm'][indx], 2))
    print(area, indx, 'cat', np.round(comparisons[area]['cat']['d_prime'][
        indx], 2), np.round(comparisons[area]['cat']['mean'][indx], 2),
          np.round(comparisons[area]['cat']['mean_perm'][indx], 2))
for area in comparisons:
    if expstr28 not in all_ps[area]['expstr']:
        continue
    indx = np.where(np.array(all_ps[area]['expstr']) == expstr28)[0][0]
    print(area, indx, 'desc', np.round(comparisons[area]['desc']['d_prime'][
        indx], 2), np.round(comparisons[area]['desc']['mean'][indx], 2),
          np.round(comparisons[area]['desc']['mean_perm'][indx], 2))
    print(area, indx, 'cat', np.round(comparisons[area]['cat']['d_prime'][
...
axs.plot(-0.35+(np.random.uniform(size=len(comparisons[area]['desc']['d_prime']))-0.5)*0.3,
             comparisons[area]['desc']['d_prime'], 'ko', alpha=0.5)
axs.boxplot(np.array(comparisons[area]['desc']['d_prime'])[
    np.isfinite(comparisons[area]['desc']['d_prime'])], positions=[-0.35],
            notch=True, bootstrap=10000, widths=0.65, showfliers=False,
            whiskerprops=dict(lw=0), capprops=dict(lw=0))
axs.plot(0.35+(np.random.uniform(size=len(comparisons[area]['cat']['d_prime']))-0.5)*0.3,
         comparisons[area]['cat']['d_prime'], 'bo', alpha=0.5)
axs.boxplot(np.array(comparisons[area]['cat']['d_prime'])[
    np.isfinite(comparisons[area]['cat']['d_prime'])], positions=[0.35],
            notch=True, bootstrap=10000, widths=0.65, showfliers=False,
            whiskerprops=dict(lw=0), capprops=dict(lw=0))
axs.set_xticklabels(['exemplar', 'category'])
# axs.set_ylabel(r"$d^{'}$")
axs.set_ylabel(r"$d'$")
fig.subplots_adjust(left=0.2)
fig.savefig('./figs/ReberEtAl2019_all_dp.pdf')



    
areas = [b'AM', b'EC', b'HC', b'PHC', b'other', 'all']
...
fig, axs = plt.subplots(1, 1)
for i, area in enumerate(areas):
    axs.plot(i*2-0.35+(np.random.uniform(
        size=len(comparisons[area]['desc']['t']))-0.5)*0.3,
             comparisons[area]['desc']['t'], 'ko', alpha=0.5)
    axs.boxplot(np.array(comparisons[area]['desc']['t'])[
        np.isfinite(comparisons[area]['desc']['t'])], positions=[i*2-0.35],
                notch=True, bootstrap=10000, widths=0.65, showfliers=False,
                whiskerprops=dict(lw=0), capprops=dict(lw=0))
    axs.plot(i*2+0.35+(np.random.uniform(
        size=len(comparisons[area]['cat']['t']))-0.5)*0.3,
             comparisons[area]['cat']['t'], 'bo', alpha=0.5)
    axs.boxplot(np.array(comparisons[area]['cat']['t'])[
        np.isfinite(comparisons[area]['cat']['t'])], positions=[i*2+0.35],
                notch=True, bootstrap=10000, widths=0.65, showfliers=False,
                whiskerprops=dict(lw=0), capprops=dict(lw=0))
...
for subj in np.unique(data['cluster_lookup'][:]['subjid']):
    print(subj, np.unique(data['cluster_lookup'][:][data['cluster_lookup'][:]['subjid']==subj]['sessid']))
