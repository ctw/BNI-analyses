...
dat_files = {}
dat_files['oi'] = sorted(glob(data_dir+'/s???/data/s*e*oi/OILog_*'))
...
for datfile in dat_files['oi']:
        ...
        for line in f:
            ...
            if game_dict_parse:
                ...
                #         exp_dict[code] = tmp_dict.copy()
                    ...
                    tmp_dict = {}
                    if 'Started' in line:
                        tmp_dict['evtype'] = 'oi_start'
                    elif 'Stopped' in line:
                        tmp_dict['evtype'] = 'oi_stop'
                    else:
                        raise ValueError('Unknown exp parameter: '+line)
                    if code in exp_dict:
                        assert exp_dict[code] == tmp_dict, \
    ...
    if exp_dict[sessevs_raw_rec[indx]['code']]['evtype'] != 'oi_start':
    ...
    while indx < len(sessevs_raw_rec):
        ...
        sessevs.append(
            (exp, subj, expnum, sess, expstr, ev_dict['stim_time'],
             ...
             ev_dict['phase3_time'], ev_dict['phase4_time'], 'oi_trial',
...
np.savez_compressed('./data/bni_evs_oi.npz', evs=evs)
...
for sf in np.unique(evs['stim_file']):
    if sf == 'MountRushmore_txt.jpg':
        continue
    if '_txt' in sf:
        if sf[:3].upper() in cats2:
            continue
        else:
            cats2[sf[:3].upper()] = sf.split('_txt')[0]
...
# evs = np.lib.recfunctions.append_fields(evs, 'stim_category2', cat2_list, usemask=False, asrecarray=True)
# np.savez_compressed('./data/bni_evs_oi.npz', evs=evs)
