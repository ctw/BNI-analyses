...
evs = np.load(
    '/home/ctw/Christoph/Analyses/BNI/oi/data/bni_evs_oi.npz')['evs'].view(
        np.recarray)
...
bad_expstr = ['s20e3oi', 's23e8oi']  # bad timing
# clusterinfo_adjust = ['s26e3oi', 's26e9oi','s27e4oi']
...
# all_ps = pickle.load(open('./data/all_ps_zmean.pickle', 'rb'))
# all_ps = pickle.load(open('./data/all_ps_zmean_cat2.pickle', 'rb'))
...
for i, (sm, spm) in enumerate(zip(all_ps['all']['sim_means'],
    ...
    # alldat = np.r_[sm['description'], spm['description']]
    # x = (np.sort(sm['description'])-alldat.mean())/alldat.std()
    x = ((np.sort(sm['category'])-spm['category'].mean())/
         spm['category'].std())
    y = np.arange(len(x))/len(x)
    axs[i%rows, col].plot(x, y, 'r-')
    # x = (np.sort(spm['description'])-alldat.mean())/alldat.std()
    x = ((np.sort(spm['category'])-spm['category'].mean())/
         spm['description'].std())
    y = np.arange(len(x))/len(x)
    axs[i%rows, col].plot(x, y, 'b--')
