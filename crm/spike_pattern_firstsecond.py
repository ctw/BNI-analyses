...
evs = np.load('/home/ctw/Christoph/Analyses/BNI/crm/data/bni_evs_crm.npz')['evs'].view(
    np.recarray)
...
for area in all_ps:
    ...
    sm = np.array(all_ps[area]['sim_mean'])
    spm = np.array(all_ps[area]['pmean'])      
    comparisons[area]['desc']['ks'].append(
        ks_2samp(sm, spm))
    comparisons[area]['desc']['mean'].append(np.nanmean(sm))
    comparisons[area]['desc']['mean_perm'].append(np.nanmean(spm))
    comparisons[area]['desc']['std_perm'].append(np.nanstd(spm))
    ...
    comparisons[area]['desc']['n'].append(len(sm))
