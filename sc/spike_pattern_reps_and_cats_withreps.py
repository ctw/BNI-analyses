...
for i, expstr in enumerate(np.unique(evs['expstr'])):
    ...
    for rep in np.unique(ev_sess['repetition']):
        ...
        bins[rep] = []
        indices[rep] = np.zeros(len(events[1]), dtype=np.int)-1
    # events['first'] = ev_sess[first_filt]
    # sme_resp = np.empty(np.sum(sme_filt), dtype = 'U3')
    # sme_bins = {'old': [], 'new': []}
    # bins = {'first': [], 'second': []}
    # second_indices = np.zeros(first_filt.sum(), dtype=np.int)-1
    ...
    for clusterfile, nsefile in zip(
        ...
        for cluster_num in np.unique(sess_clusts):
            ...
            for rep in bins:
                ...
                # the indices for the repeated presentations do not, so we
                # need to deal with this manually:
                hist, _ = np.histogram(clustimes, sorted(bins[rep]))
                indxsort = np.argsort(bins[rep])
                # every other bin is the time between item presentations,
                # so we're skipping these:
                spikesums[ci['area'][0]][rep].append(hist[indxsort[::2]])
    ...
    for area in channels:
        for rep in spikesums[area]:
            spikesums[area][rep] = np.array(spikesums[area][rep])
            if len(spikesums[area][rep].shape) == 1:
                spikesums_norm[area][rep] = spikesums[area][rep]
            else:
                spikesums_norm[area][rep] = spikesums[area][
                    rep]/np.atleast_2d(spikesums[area][rep].max(1)).T
                spikesums_norm[area][rep][spikesums[area][rep].max(1) == 0] = 0
            firing_rate[area][rep] = spikesums[area][rep]/(
                timewindowlength/1000000)
            times[area][rep] = np.array(bins[rep][::2]) + timewindowlength/2
...
for area in all_areas:
    ...
    for i, av in enumerate(all_vecs):
        ...
        if len(av['firing_rate'][area][1].shape) != 2:
            print('firing_rate shape: ', len(av['firing_rate'][area][1].shape))
            continue
        norm_vecs = {}
        ...
        for i, rep in enumerate(reps[:-1]):
            ...
            for rep2 in reps[i+1:]:
                sim_mean[rep][rep2] = np.nanmean(np.diag(np.dot(
                    norm_vecs[rep].T, norm_vecs[rep2])))
                sim_perm_mean[rep][rep2] = np.ones(permutations)*np.nan
                for p in range(permutations):
                    rep2_perm = rng.permutation(norm_vecs[rep2], 1)
                    sim_perm_mean[rep][rep2][p]=np.nanmean(np.diag(np.dot(
                        norm_vecs[rep].T, rep2_perm)))
        all_ps[area]['pval'].append(
            {rep: {rep2: np.sum(
                sim_mean[rep][rep2]<sim_perm_mean[rep][rep2])/permutations
                   for rep2 in sim_mean[rep]} for rep in sim_mean})
        all_ps[area]['pmean'].append(
            {rep: {rep2: sim_perm_mean[rep][rep2].mean()
                   for rep2 in sim_perm_mean[rep]} for rep in sim_perm_mean})
        all_ps[area]['pstd'].append(
            {rep: {rep2: sim_perm_mean[rep][rep2].std()
                   for rep2 in sim_perm_mean[rep]} for rep in sim_perm_mean})
...
for i, av in enumerate(all_vecs):
    ...
    for rep in av['firing_rate'][all_areas[0]]:
        all_fr = np.concatenate([av['firing_rate'][area][
            rep] for area in all_areas if len(av['firing_rate'][area][
                rep].shape)==2])
        norm_vecs[rep] = all_fr/np.sqrt((all_fr**2).sum(0))
    sim_mean = {}
    sim_perm_mean = {}
    for i, rep in enumerate(reps[:-1]):
        sim_mean[rep] = {}
        ...
        for rep2 in reps[i+1:]:
            ...
            for p in range(permutations):
                ...
                sim_perm_mean[rep][rep2][p]=np.nanmean(np.diag(np.dot(
                    norm_vecs[rep].T, rep2_perm)))
    all_ps['all']['pval'].append(
        {rep: {rep2: np.sum(
            sim_mean[rep][rep2]<sim_perm_mean[rep][rep2])/permutations
               for rep2 in sim_mean[rep]} for rep in sim_mean})
    all_ps['all']['pmean'].append(
        {rep: {rep2: sim_perm_mean[rep][rep2].mean()
               for rep2 in sim_perm_mean[rep]} for rep in sim_perm_mean})
    all_ps['all']['pstd'].append(
        {rep: {rep2: sim_perm_mean[rep][rep2].std()
               for rep2 in sim_perm_mean[rep]} for rep in sim_perm_mean})
